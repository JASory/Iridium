...
/// ```
   ...
   /// Transforms a nuclide from the unique index.
    ...
    /*
    /// Partial Halflife f64::NAN if not supported f64::Infinity if stable
   pub fn partial_half_life<T: DecayMode>(&self) -> f64{
     let branch = self.branching_ratio::<T>();
     let halflife = HALF_LIFE[self.nuclide_index()];
     halflife/branch
   } 
   
   pub fn partial_mean_lifetime<T: DecayMode>(&self) -> f64 {
        //reciprocal of ln(2) average lifespan of a particle
        self.partial_half_life::<T>() * std::f64::consts::LOG2_E
    }

    /// Approximation of decay constant
    pub fn partial_decay_constant<T: DecayMode>(&self) -> f64 {
        self.partial_mean_lifetime::<T>().recip()
    }
    */
    ...
    /*
    pub fn daughter_energetic<T: DecayMode>(&mut self) -> (f64, Vec<Particle>){
        T::decay(self)
    }
    */
    /*
